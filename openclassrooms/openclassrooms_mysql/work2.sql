...
-- PAGE ACCUEIL --
select article.id, article.titre, article.resume, date_format(article.date_publication, '%d/%m/%Y') as date_pub,
		   utilisateur.pseudo as user_pseudo, count(Commentaire.article_id) as nb_commentaire
from Article
...
-- PAGE UTILISATEUR --
select utilisateur.pseudo, utilisateur.email, article.titre, article.resume, article.contenu,
		  date_format(article.date_publication, '%e %M ''%y') as date_pub
from utilisateur
...
-- PAGE CATEGORIE --
select Categorie.nom as categorie, article.titre, article.resume, article.contenu,
          date_format(article.date_publication, '%d/%m/%Y - %H:%i'), utilisateur.pseudo as user_pseudo
from Categorie
left join Categorie_article as intermediaire on Categorie.id = intermediaire.categorie_id
inner join article on article.id = intermediaire.article_id
...
-- PAGE ARTICLE --
select article.id, article.titre, article.contenu, date_format(article.date_publication, '%e %M %Y à %k heures %i'),
		  utilisateur.pseudo as user_pseudo, group_concat(categorie.nom) as categories
from article
inner join utilisateur on article.auteur_id = utilisateur.id
left join Categorie_article as intermediaire on article.id = intermediaire.article_id
inner join categorie on intermediaire.categorie_id = categorie.id
where article.id = 4
group by article.id;
