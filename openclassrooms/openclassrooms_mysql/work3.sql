...
-- Vue materialisé --
CREATE TABLE VM_article_nb_commentaire
ENGINE = InnoDB
select article.id as article_id, count(Commentaire.article_id) as nb_commentaire
FROM Article
left join Commentaire on Commentaire.article_id = Article.id
group by Commentaire.article_id, Article.id;


alter table VM_article_nb_commentaire
add constraint fk_vb_article_nb_commentaire_article_id foreign key (article_id) references Article (id) on delete cascade,
add primary key (article_id);


create table VM_article_nb_last_date_by_user (
    id_utilisateur int unsigned primary key
)
ENGINE = InnoDB
select utilisateur.id as id_utilisateur,  count(*) as nb_article, max(date_publication) as last_article_date
from article
inner join utilisateur on article.auteur_id = utilisateur.id
group by utilisateur.id;


create table VM_commentaire_nb_last_date_by_user (
    id_utilisateur int unsigned primary key
)
ENGINE = InnoDB
select utilisateur.id as id_utilisateur,  count(*) as nb_commentaire, max(date_commentaire) as last_commentaire_date
from commentaire
left join utilisateur on commentaire.auteur_id = utilisateur.id
where Commentaire.auteur_id is not null
group by utilisateur.id;
...
begin
	update VM_article_nb_commentaire
...
begin
	insert into VM_article_nb_commentaire (article_id, nb_commentaire)
    values (new.id, 0);
end|
delimiter ;
...
-- procédure stocké --
DELIMITER |
CREATE PROCEDURE maj_vm_couters_date_by_user()
BEGIN
		truncate VM_article_nb_last_date_by_user;
        truncate VM_commentaire_nb_last_date_by_user;
        
        insert into VM_article_nb_last_date_by_user
        select utilisateur.id as id_utilisateur,  count(*) as nb_article, max(date_publication) as last_article_date
		from article
		inner join utilisateur on article.auteur_id = utilisateur.id
		group by utilisateur.id;
        
        insert into VM_commentaire_nb_last_date_by_user
        select utilisateur.id as id_utilisateur,  count(*) as nb_commentaire, max(date_commentaire) as last_commentaire_date
		from commentaire
		left join utilisateur on commentaire.auteur_id = utilisateur.id
		where Commentaire.auteur_id is not null
...
select article.id, article.titre,
		  case article.resume
          when null then concat(substr(Article.contenu, 1, 150), '...')
          else Article.resume
          end as resume,
		  date_format(article.date_publication, '%d/%m/%Y') as date_pub,
		   utilisateur.pseudo as user_pseudo, article_nb_com.nb_commentaire
from Article
inner join utilisateur on Article.auteur_id = utilisateur.id
inner join VM_article_nb_commentaire as article_nb_com on Article.id = article_nb_com.article_id
order by article.date_publication desc;


-- page administration --
select id_utilisateur, utilisateur.pseudo, nb_commentaire, last_commentaire_date 
from VM_commentaire_nb_last_date_by_user
inner join Utilisateur on Utilisateur.id = VM_commentaire_nb_last_date_by_user.id_utilisateur;

select id_utilisateur, utilisateur.pseudo, nb_article, last_article_date 
from VM_article_nb_last_date_by_user
inner join Utilisateur on Utilisateur.id = VM_article_nb_last_date_by_user.id_utilisateur;
